...
var sym_rg *regexp.Regexp = regexp.MustCompile("[*]")
...
func main() {
	...
	inp6 := map[int][]int{}
	inp4, inp5 := proc5(inp1, inp3, len(inp[len(inp)-1]), len(inp))
	for i, e := range inp4 {
		if e {
			// fmt.Println(inp5[i], " ", proc6(inp1[i], inp))

			val, ok := inp6[inp5[i]]
			if ok {
				val = append(val, proc6(inp1[i], inp))
				inp6[inp5[i]] = val
			} else {
				inp6[inp5[i]] = []int{ proc6(inp1[i], inp) }
			}
			// fmt.Println(inp5[i], " ", inp6[inp5[i]])
		}
	}
	fmt.Println("===========")
...
func proc1(inp []int, sym_locs []int, line_count int, inp_count int) (bool, int) { //input is start, end index, then the line it is in; then the entire splitted input. finds if there's a symbol around
...
func proc5(inp [][]int, sym_locs []int, line_count int, inp_count int) ([]bool, []int) {
	res := []bool{}
	res1 := []int{}
	for _, e := range inp {
		amogus_bool, amogus_ind := proc1(e, sym_locs, line_count, inp_count)
		res = append(res, amogus_bool)
		res1 = append(res1, amogus_ind)
	}
...
func proc6(inp []int, inp_txt []string) int { //retrieve number from position
	txt := inp_txt[inp[2]]
	txt1 := txt[inp[0]:inp[1]]

	res, err := strconv.Atoi(txt1)
	if err != nil { os.Exit(1) }
	return res
}

func proc7(inp []int) int {
	if len(inp) == 2 {
		return inp[0] * inp[1]
	} 
	return 0
